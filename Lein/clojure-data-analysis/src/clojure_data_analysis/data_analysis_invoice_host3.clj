(ns clojure-data-analysis.data-analysis-invoice-host3
...
;count invoice by lot
(defn count-invoice-by-lot [lot]
  (mc/count database collection-name {:batchId lot}))

;find invoice by lot and status
(defn count-invoice-by-lot-and-status [lot, status]
  (mc/count database collection-name {$and [{:batchId lot}
                           {:status status}]}))

(println (find-invoice-by-batch-id 1))
(println "Total of invoices" (count-invoice-by-status invoice-created-status))
...
(println "Total of invoices with lot 6: and status" invoice-created-status (count-invoice-by-lot-and-status 6 invoice-created-status))
